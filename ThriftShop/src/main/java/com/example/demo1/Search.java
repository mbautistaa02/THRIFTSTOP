...
public class Search extends prendaprevController {
    ...
    private static List<String> historialDeBusquedas = new ArrayList<>();

    public static void agregarAlHistorial(String query) {
        String[] keywords = query.toLowerCase().split(" ");
        historialDeBusquedas.addAll(Arrays.asList(keywords));
    }

    private static List<Prenda> prendasBuscadas = new ArrayList<>();

    public void agregarPrendaBuscada(Prenda prenda) {
        if (!prendasBuscadas.contains(prenda)) { // Evitar duplicados
            prendasBuscadas.add(prenda);
        }
    }
    @FXML
    private void mostrarRecomendaciones() {
        Prenda prendaAleatoria;
        List<Prenda> prendas = HelloApplication.getListaDePrendas();
        Graph graph = new Graph();
        if(!prendasBuscadas.isEmpty()){
            graph.buildGraphbySize(prendasBuscadas);
        }
        else{
        ...
        if(!prendasBuscadas.isEmpty()){
            prendaAleatoria = prendasBuscadas.get(random.nextInt(prendasBuscadas.size()));}
        else{
            prendaAleatoria = prendasGrafo.get(random.nextInt(prendasGrafo.size()));
        }

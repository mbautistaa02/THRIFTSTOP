...
public class Graph {
    ...
    public void buildGraphbySize(List<Prenda> prendasFiltradas) {
        for (Prenda prenda : HelloApplication.getListaDePrendas()) {
            addPrenda(prenda);
        }

        for (int i = 0; i < prendasFiltradas.size(); i++) {
            for (int j = i + 1; j < HelloApplication.getListaDePrendas().size(); j++) {
                Prenda prenda1 = prendasFiltradas.get(i);
                Prenda prenda2 = HelloApplication.getListaDePrendas().get(j);

                if (prenda1.getTalla().equals(prenda2.getTalla()) || prenda1.getNombre().equals(prenda2.getNombre()) || prenda1.getPrecio()==(prenda2.getPrecio() )) {
                    connectPrenda(prenda1, prenda2);
                    System.out.println("Connected: " + prenda1 + " <-> " + prenda2);
                }
            }
        }
    }
